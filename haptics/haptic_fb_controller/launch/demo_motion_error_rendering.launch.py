...
def launch_setup(context, *args, **kwargs):
    ...
    launch_simulation = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            "controllers_file": "slave_controllers_motion_error_rendering.yaml",
            "spawn_z": "1.03",
    ...
    # Haptic virtual force controller spawner
    haptic_virtual_force_controller_spawner = Node(
        package="controller_manager",
        executable="spawner",
        arguments=["haptic_virtual_force_controller", "-c", "/hd/controller_manager"],
        namespace="hd",
    )
    ...
    # If dynamic simulation is true, launch the haptic feedback controller with the dynamic simulation
    first_controller_spawner_with_delay = TimerAction(
        period=20.0,
        actions=[haptic_fb_controller_spawner],
    )

    # After feedback controller exits, launch the admittance controller
    start_next_controller = RegisterEventHandler(
        event_handler=OnProcessExit(
            target_action=haptic_fb_controller_spawner,
    ...
    layout_path = PathJoinSubstitution(
        [
            FindPackageShare("haptic_fb_controller"),
            "config",
            "plot_raw_forces_and_actuated_forces_config_motion_error_rendering.xml",
        ]
    )
