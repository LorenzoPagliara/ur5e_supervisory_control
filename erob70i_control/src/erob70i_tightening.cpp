...
int main(int argc, char **argv)
{
	...
	*/
	...
	double sum = 0.0;
	u_int16_t avgTor0 = 0.0;
	int counter = 0;
	bool is_threshold_reached = false;
	...
	while (1)
	{
		...
		*/
		...
		actTor0 = EC_READ_S16(domain1_pd + actual_torque);
//
		...
		if (std::abs(actTor0) < 100)
		{
			...
			avgTor0 = static_cast<uint16_t>(std::abs(sum / window_size));
		...
		switch(state) {
			...
			case STATE_OPERATION_ENABLED:
				...
				if (avgTor0 < 95 && !is_threshold_reached)
				{
					// CSV mode operation
					actVel0 = EC_READ_S32(domain1_pd + actual_velocity);
					
					// Set constant target velocity
					EC_WRITE_S32(domain1_pd + target_velocity, -40000);
				...
				else
				{
					...
					{
						printf("Torque threshold reached, stopping drive\n");
